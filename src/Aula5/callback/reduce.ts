...
// Exemplo 2: Agrupar e contar a quantidade de produtos por categoria
type QuantidadePorCategoria3 = Record<string, number>;

const contagemCategorias = produtos3.reduce<QuantidadePorCategoria3>((acc, p) => {
  if (!acc[p.categoria]) {
    acc[p.categoria] = 0;
  }
  acc[p.categoria]++;
